...
### 4.5 上下文注入（修改 `system_prompt`）

按顺序叠加到智能体系统提示前（**不含**跨会话 `history_summary`，见 §4.1「memory_search」）：

1. **debug_options**（含 `injected_context`：页面信息、用户身份等）
2. **挂载的技能**：读取 `SKILLS_DIR/{id}/SKILL.md` 全文注入
3. **长期记忆 LTM**：短超时拉取用户 facts（`ltm_service`）并写入 system；用户偏好也可由模型调用 `fetch_user_long_term_memory` 再查
4. **用户画像 system 消息**：账号名、部门、礼仪与安全规则（`_build_user_context_msg`）

跨会话「最近聊了啥」：**不**在此处自动注入；由模型在需要时调用 **`memory_search`**（设计见 §4.1）。

`AgentContextManager.setup_context` 将 `user_id`、`admin`、`api_key` 写入运行时上下文，供工具与 SQL 改写使用。
...
| `LOCAL`（默认） | 意图识别后分支 | 见下表 |

**LOCAL 分支**：

| 条件 | 执行器 |
|------|--------|
| 意图为 `data_query` 且智能体具备 `data_query` 能力 | `DataQueryExecutor` |
| 其他 | `GeneralChatExecutor` |
...
| 执行器 | `app/services/ai/executors/` |
| 会话记忆 | `app/services/ai/memory_service.py` |
| 会话摘要 / 每日摘要 / 索引 | `app/services/ai/session_summary_service.py`、`daily_summary_service.py`、`memory_index_service.py`、`conversation_summarizer.py` |
| Embedding | `app/services/ai/embedding_client.py` |
| 长期记忆工具 | `app/services/ai/tools/memory_ltm_tools.py` |
| 跨会话检索工具 | `app/services/ai/tools/memory_search_tool.py` |
| 记忆配置 | `app/services/memory_config_service.py` |
| 记忆 Portal API | `app/api/portal/endpoints/memory.py` |
| Redis 向量自检 | `app/services/ai/redis_vector_health.py` |
...
| 智能路由设计（补充） | `architech/design/AGENT_ROUTING_DESIGN.md` |
| AI 系统总览（补充） | `architech/design/AI_AGENT_SYSTEM_DESIGN.md` |
...
*文档版本：2026-05-28。跨会话 session summary、daily summary、`memory_search`、记忆管理中心、会话 `finalize` 与 Redis 向量环境检测**已实现**；会话空闲超时归档摘要仍为二期。规格见 `docs/superpowers/specs/2026-05-27-memory-search-redis-stack-design.md`。*

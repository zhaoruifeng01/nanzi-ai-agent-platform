...
SET @yunshu_turn_classify_block =
...
SET @yunshu_old_meta_note =
'> ⚙️ **元操作例外 (Meta-Action Exception)**：当用户请求的是基于**已有对话/上一轮结果**的"管理类操作"（例如：创建技能、保存为技能或模板、把流程固定下来、记住某项偏好），且本身**不需要查询新的业务数据**时，禁止机械地重新执行 `get_dataset_schema` / `execute_sql_query`。应直接基于已有上下文完成，或调用对应的管理工具（如 `create_skills`）。这类请求不属于数据查询，**不受**下文"禁止直接回答 / 禁止跳过元数据查询"等约束。';
...
UPDATE `ai_agent_versions`
SET `system_prompt` = REPLACE(`system_prompt`, '{dataset_menu}', CONCAT('{dataset_menu}\n\n', @yunshu_turn_classify_block))
WHERE `agent_id` = 'sys-agent-chatbi'

...
def test_resolve_last_month_expectation():
    exp = resolve_relative_time_expectation("帮我查上个月拜访记录", now=NOW)
...
def test_detect_mismatch_last_month_wrong_year():
    ...
    reason = detect_time_range_mismatch("查询上个月拜访记录", sql, now=NOW)
    assert reason
    assert "上月" in reason
...
def test_detect_pass_correct_last_month():
    ...
    assert detect_time_range_mismatch("查询上个月拜访记录", sql, now=NOW) == ""
...
def test_skip_when_user_gives_explicit_absolute_time():
    ...
    assert detect_time_range_mismatch("查询2025年5月拜访记录", sql, now=NOW) == ""
...
def test_last_year_allows_previous_calendar_year():
    ...
    assert detect_time_range_mismatch("统计去年销售额", sql, now=NOW) == ""

...
统一服务不消费异步流、不发送 SSE、不调用工具，也不修改已输出正文。GENERAL 和 ChatBI 仍先流式输出正文并在结束后追加可选提示；需要完整审核的 Main 请求维持原缓冲方式；KnowledgeAgent 维持原反思循环。因此服务化不会改变正文到达顺序或首字延迟。
...
8. `GENERAL` 若输出“当前、最新、实时”等高置信动态事实，执行轻量事实复核；普通解释继续直接流式输出。

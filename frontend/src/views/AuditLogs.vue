        ...
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="log in logs" :key="log.id" class="hover:bg-gray-50 transition-colors">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium"><button @click="viewLogDetail(log.id)" class="text-blue-600 hover:text-blue-900 font-bold">查看</button></td>
            </tr>
...
const viewLogDetail = async (logId: number) => {
  ...
  try {
    const res = await axios.get(`${API_BASE}/api/portal/audit/logs/${logId}`, { headers: { "X-API-Key": apiKey.value } });

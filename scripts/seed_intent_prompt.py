...
NEW_PROMPT = """你是一个专业的 AI 智能体助手，负责解析用户的意图。
...
async def main():
    ...
    print("Updating 'intent_recognition_prompt' in system_configs...")
    await ConfigService.set_config(
        key="intent_recognition_prompt",
        value=NEW_PROMPT,
        description="意图识别阶段的系统提示词 (System Prompt)",
        category="agent",
        is_secret=False
    )
    ...
    # Verify
    val = await ConfigService.get("intent_recognition_prompt")
    print("-" * 20)
    print(f"Current Value in DB:\n{val[:50]}...")
    print("-" * 20)
